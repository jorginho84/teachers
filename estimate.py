...
class estimate:
    ...
    def simulation(self,times,modelSD):
        ...
        for i in range(1,times):
            ...
            #est_mean_Port[i] = np.mean(opt['Opt Teacher'][0])
            est_mean_Port[i] = np.mean(datav['PORTFOLIO'].to_numpy())
            #est_var_Port[i] = np.var(opt['Opt Teacher'][0])
            est_var_Port[i] = np.var(datav['PORTFOLIO'].to_numpy())
            #est_mean_Pru[i] = np.mean(opt['Opt Teacher'][1])
            est_mean_Pru[i] = np.mean(datav['TEST'].to_numpy())
            #est_var_Pru[i] = np.var(opt['Opt Teacher'][1])
            est_var_Pru[i] = np.var(datav['TEST'].to_numpy())
            perc_init[i] = (sum(datav['PLACEMENT']==1) / len(datav['PLACEMENT'])) * 100
            perc_inter[i] = (sum(datav['PLACEMENT']==2) / len(datav['PLACEMENT'])) * 100
            perc_advan[i] = (sum(datav['PLACEMENT']==3) / len(datav['PLACEMENT'])) * 100
            perc_expert[i] = ((sum(datav['PLACEMENT']==4)+sum(datav['PLACEMENT']==5)) / len(datav['PLACEMENT'])) * 100
            ...
            est_mean_SIMCE[i] = np.mean(datav0['SIMCE'].to_numpy())
        ...
        return {'Estimation SIMCE vs Portfolio': est_bootsSPort,
            ...
            'Mean SIMCE': est_sim_mean_SIMCE,
                'Error SIMCE vs Portfolio': error_bootsSPort,
                'Error SIMCE vs Prueba': error_bootsSPrue}
    ...
    def objfunction(self,beta):
        ...
        self.param0.alphas[1][0] = beta[4]
        self.param0.alphas[1][1] = beta[5]
        ...
        self.param0.betas[3] = beta[11]
        self.param0.gammas[0] = beta[12]
        self.param0.gammas[1] = beta[13]
        self.param0.gammas[2] = beta[14]
        ...
        beta_portpru = result['Estimation Portfolio vs Prueba']
        ...
        num_par = beta_sport.size +beta_spru.size + beta_portpru.size + beta_mport.size + beta_mtest.size + \
            beta_expport.size + beta_exptest.size + beta_vport.size + beta_vtest.size + beta_pinit.size + \
                beta_pinter.size + beta_padv.size + beta_pexpert.size + beta_msimce.size + beta_pinter.size
        
        #Outer matrix
        x_vector=np.zeros((num_par,1))
        
        x_vector[0,0] = beta_sport - self.moments_vector[0,1]
        x_vector[1,0] = beta_spru - self.moments_vector[1,1]
        x_vector[2,0] = beta_portpru - self.moments_vector[2,1]
        x_vector[3,0] = beta_mport - self.moments_vector[3,1]
        x_vector[4,0] = beta_mtest - self.moments_vector[4,1]
        x_vector[5,0] = beta_expport - self.moments_vector[5,1]
        x_vector[6,0] = beta_exptest - self.moments_vector[6,1]
        x_vector[7,0] = beta_vport - self.moments_vector[7,1]
        x_vector[8,0] = beta_vtest - self.moments_vector[8,1]
        x_vector[9,0] = beta_pinit - self.moments_vector[9,1]
        x_vector[10,0] = beta_pinter - self.moments_vector[10,1]
        x_vector[11,0] = beta_padv - self.moments_vector[11,1]
        x_vector[12,0] = beta_pexpert - self.moments_vector[12,1]
        x_vector[13,0] = beta_msimce - self.moments_vector[13,1]
        x_vector[14,0] = beta_pinter - self.moments_vector[13,1]
