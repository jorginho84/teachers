...
betas_opt_t = np.array([betas_nelder[10],betas_nelder[11],betas_nelder[12],
	betas_nelder[13],betas_nelder[14]]).reshape((5,1))

se_betas_opt_t=np.array([se_vector[10],se_vector[11],se_vector[12],
                         se_vector[13],se_vector[14]]).reshape((5,1))
    ...
    for j in range(len(alphas_test_list)):
        if (j == 0) | (j == 2):
            f.write(prod_names_old[j] + r' &  &  '+ '{:04.3f}'.format(alphas_test_list[j]) +
                ...
                f.write(prod_names_old[j] + r' &  &  '+ '{:04.3f}'.format(alphas_test_list[j]) +
                        r' &  & '+ '{:1.2f}'.format(se_alphas_test[j])+r' \\' + '\n')
    f.write(r'\hline'+'\n')
