...
class Utility(object):
    ...
    def income(self, initial_p):
        ...
        # " EXPERIENCE (4 years)"
        biennium = np.zeros((initial_p_2.shape[0],4))
        
        for j in range(4):
            biennium[:,j] = np.where(np.floor((self.years + j)/2) >15, 15, np.floor((self.years + j)/2) )
        
        porc1 = self.param.porc[0] 
        porc2 = self.param.porc[1]
        
        ExpTrameE_m = np.zeros((initial_p_2.shape[0],4))
        ExpTrameS_m = np.zeros((initial_p_2.shape[0],4))
        
        for j in range(j):
            ExpTrameE_m[:,j] = np.where((self.typeSchool==1) & (self.years + j > 2), (porc1 + (porc2 * (biennium[:,j] - 1))) * RBMNElemt, ExpTrameE2)
            ExpTrameS_m[:,j] = np.where((self.typeSchool==0) & (self.years + j > 2), (porc1 + (porc2 * (biennium[:,j] - 1))) * RBMNSecond, ExpTrameS2)
            
        ExpTrameE = np.sum(ExpTrameE_m,axis = 1)
        ExpTrameS = np.sum(ExpTrameS_m,axis = 1)
        ...
        professional_qualificationW = 4*self.param.pro[0] 
        professional_mentionW = 4*self.param.pro[1]
        professional_qualification = self.param.pro[2]
        professional_mention = 4*self.param.pro[3]
        full_contract = 44
        ...
        ATDPadvancedfixed = 4*(Proadvancedfixed)*(self.HOURS/full_contract)
        ATDPexpert1fixed = 4*(Proexpert1fixed)*(self.HOURS/full_contract)
        ATDPexpert2fixed = 4*(Proexpert2fixed)*(self.HOURS/full_contract)

        
        ATDPinitial_m = np.zeros((initial_p_2.shape[0],4))
        ATDPearly_m = np.zeros((initial_p_2.shape[0],4))
        ATDPadvanced_m = np.zeros((initial_p_2.shape[0],4))
        ATDPexpert1_m = np.zeros((initial_p_2.shape[0],4))
        ATDPexpert2_m = np.zeros((initial_p_2.shape[0],4))

        for j in range(4):
            ATDPinitial_m[:,j] = (Proinitial/15)*(self.HOURS/full_contract)*biennium[:,j]
            ATDPearly_m[:,j] = (ProEarly/15)*(self.HOURS/full_contract)*biennium[:,j]
            ATDPadvanced_m[:,j] = (Proadvanced/15)*(self.HOURS/full_contract)*biennium[:,j]
            ATDPexpert1_m[:,j] = (Proexpert1/15)*(self.HOURS/full_contract)*biennium[:,j]
            ATDPexpert2_m[:,j] = (Proexpert2/15)*(self.HOURS/full_contract)*biennium[:,j]


        ATDPinitial = np.sum(ATDPinitial_m,axis = 1)
        ATDPearly = np.sum(ATDPearly_m,axis = 1)
        ATDPadvanced = np.sum(ATDPadvanced_m,axis = 1)
        ATDPexpert1 = np.sum(ATDPexpert1_m,axis = 1)
        ATDPexpert2 = np.sum(ATDPexpert2_m,axis = 1)
        
        
         
        # " AEP (Teaching excellence) (4 years)
        
        AcreditaTramoI = 4*self.param.AEP[0]
        AcreditaTramoII = 4*self.param.AEP[1]
        AcreditaTramoIII = 4*self.param.AEP[2]
    
            
        # " Asignación de perfeccionamiento
        # " This is the new asignment
        AsigElemt_m = np.zeros((initial_p_2.shape[0],4))
        AsigSecond_m = np.zeros((initial_p_2.shape[0],4))

        for j in range(4):
            AsigElemt_m[:,j] = np.where(((self.typeSchool==1)), RBMNElemt*0.4*(biennium[:,j]/15), AsigElemt2)
            AsigSecond_m[:,j] = np.where((self.typeSchool==0), RBMNSecond*0.4*(biennium[:,j]/15), AsigSecond2)

        AsigElemt = np.sum(AsigElemt_m, axis = 1)
        AsigSecond = np.sum(AsigSecond_m, axis = 1)

        
        # " Asignación por docencia
        # "Post-reform Priority allocation

        prioirtyap_m = np.zeros(((initial_p_2.shape[0],4)))

        for j in range(4):
        
            prioirtyap1 = np.where(((self.priotity >= 0.6) & (self.priotity < 0.8)) & (initial_p_2==1), ((ATDPinitial_m[:,j]+ExpTrameE_m[:,j])*0.2)+((self.HOURS/full_contract)*self.param.priori[0]), prioirtyap11)
            prioirtyap2 = np.where(((self.priotity >= 0.6) & (self.priotity < 0.8)) & (initial_p_2==2), (((ATDPearly_m[:,j]+ExpTrameE_m[:,j])*0.2)+((self.HOURS/full_contract)*self.param.priori[0])), prioirtyap22)
            prioirtyap3 = np.where(((self.priotity >= 0.6) & (self.priotity < 0.8)) & (initial_p_2==3), ((ATDPadvanced_m[:,j]+ExpTrameE_m[:,j])*0.2)+((self.HOURS/full_contract)*self.param.priori[0]), prioirtyap33)
            prioirtyap4 = np.where(((self.priotity >= 0.6) & (self.priotity < 0.8)) & (initial_p_2==4), ((ATDPexpert1_m[:,j]+ExpTrameE_m[:,j])*0.2)+((self.HOURS/full_contract)*self.param.priori[0]), prioirtyap44)
            prioirtyap5 = np.where(((self.priotity >= 0.6) & (self.priotity < 0.8)) & (initial_p_2==5), ((ATDPexpert2_m[:,j]+ExpTrameE_m[:,j])*0.2)+((self.HOURS/full_contract)*self.param.priori[0]), prioirtyap55)
            prioirtyap51 = np.where((self.priotity >= 0.8) & (initial_p_2==1), ((ATDPinitial_m[:,j]+ExpTrameE_m[:,j])*0.2)+((self.HOURS/full_contract)*self.param.priori[1]), prioirtyap661)
            prioirtyap52 = np.where((self.priotity >= 0.8) & (initial_p_2==2), ((ATDPearly_m[:,j]+ExpTrameE_m[:,j])*0.2)+((self.HOURS/full_contract)*self.param.priori[1]), prioirtyap771)
            prioirtyap6 = np.where((self.priotity >= 0.8) & (initial_p_2==3), ((ATDPadvanced_m[:,j]+ExpTrameE_m[:,j])*0.2)+((self.HOURS/full_contract)*self.param.priori[2]), prioirtyap66)
            prioirtyap7 = np.where((self.priotity >= 0.8) & (initial_p_2==4), ((ATDPexpert1_m[:,j]+ExpTrameE_m[:,j])*0.2)+((self.HOURS/full_contract)*self.param.priori[2]), prioirtyap77)
            prioirtyap8 = np.where((self.priotity >= 0.8) & (initial_p_2==5), ((ATDPexpert2_m[:,j]+ExpTrameE_m[:,j])*0.2)+((self.HOURS/full_contract)*self.param.priori[2]), prioirtyap88)
            prioirtyap91 = np.where(((self.priotity >= 0.45) & (self.priotity < 0.6))  & (self.rural_rbd==1) & (initial_p_2==1), (ATDPinitial_m[:,j]*0.1), prioirtyap991)
            prioirtyap92 = np.where(((self.priotity >= 0.45) & (self.priotity < 0.6))  & (self.rural_rbd==1) & (initial_p_2==2), (ATDPearly_m[:,j]*0.1), prioirtyap992)
            prioirtyap93 = np.where(((self.priotity >= 0.45) & (self.priotity < 0.6))  & (self.rural_rbd==1) & (initial_p_2==3), (ATDPadvanced_m[:,j]*0.1), prioirtyap993)
            prioirtyap94 = np.where(((self.priotity >= 0.45) & (self.priotity < 0.6))  & (self.rural_rbd==1) & (initial_p_2==4), (ATDPexpert1_m[:,j]*0.1), prioirtyap994)
            prioirtyap95 = np.where(((self.priotity >= 0.45) & (self.priotity < 0.6))  & (self.rural_rbd==1) & (initial_p_2==5),(ATDPexpert2_m[:,j]*0.1), prioirtyap995)
            prioirtyap_m[:,j] = sum([prioirtyap1,prioirtyap2,prioirtyap3,prioirtyap4,prioirtyap5,prioirtyap51,prioirtyap52\
                              ,prioirtyap6,prioirtyap7,prioirtyap8,prioirtyap91,prioirtyap92,prioirtyap93,prioirtyap94,prioirtyap95])
        prioirtyap = np.sum(prioirtyap_m, axis = 1)
        ...
        localAssig_1 = 4*np.where((self.typeSchool == 1), (self.locality*RBMNElemt/100), localAssig2)
        localAssig_0 = 4*np.where((self.typeSchool == 0), (self.locality*RBMNSecond/100), localAssig3)

    
            
        # " SUM OF TOTAL SALARY "
        
        salary1 = np.where((initial_p_2==1) & (self.treatment == 1) & (self.typeSchool == 1), sum([4*RBMNElemt,ExpTrameE,BRP,ATDPinitial,prioirtyap,localAssig_1]), salary2d)
        salary3 = np.where((initial_p_2==1) & (self.treatment == 1) & (self.typeSchool == 0), sum([4*RBMNSecond,ExpTrameS,BRP,ATDPinitial,prioirtyap,localAssig_0]), salary2d)
        
        salary5 = np.where((initial_p_2==2) & (self.treatment == 1) & (self.typeSchool == 1), sum([4*RBMNElemt,ExpTrameE,BRP,ATDPearly,prioirtyap,localAssig_1]), salary2d)
        salary7 = np.where((initial_p_2==2) & (self.treatment == 1) & (self.typeSchool == 0), sum([4*RBMNSecond,ExpTrameS,BRP,ATDPearly,prioirtyap,localAssig_0]), salary2d)

        
        salary9 = np.where((initial_p_2==3) & (self.treatment == 1) & (self.typeSchool == 1), sum([4*RBMNElemt,ExpTrameE,BRP,ATDPadvanced,ATDPadvancedfixed,prioirtyap,localAssig_1]), salary2d)
        salary11 = np.where((initial_p_2==3) & (self.treatment == 1) & (self.typeSchool == 0), sum([4*RBMNSecond,ExpTrameS,BRP,ATDPadvanced,ATDPadvancedfixed,prioirtyap,localAssig_0]), salary2d)

        salary13 = np.where((initial_p_2==4) & (self.treatment == 1) & (self.typeSchool == 1), sum([4*RBMNElemt,ExpTrameE,BRP,ATDPexpert1,ATDPexpert1fixed,prioirtyap,localAssig_1]), salary2d)
        salary15 = np.where((initial_p_2==4) & (self.treatment == 1) & (self.typeSchool == 0), sum([4*RBMNSecond,ExpTrameS,BRP,ATDPexpert1,ATDPexpert1fixed,prioirtyap,localAssig_0]), salary2d)

        salary17 = np.where((initial_p_2==5) & (self.treatment == 1) & (self.typeSchool == 1), sum([4*RBMNElemt,ExpTrameE,BRP,ATDPexpert2,ATDPexpert2fixed,prioirtyap,localAssig_1]), salary2d)
        salary19 = np.where((initial_p_2==5) & (self.treatment == 1) & (self.typeSchool == 0), sum([4*RBMNSecond,ExpTrameS,BRP,ATDPexpert2,ATDPexpert2fixed,prioirtyap,localAssig_0]), salary2d)

        
        salary21 = np.where((initial_p_aep==6) & (self.treatment == 0) & (self.typeSchool == 1), sum([4*RBMNElemt,ExpTrameE,BRPWithout,AsigElemt]), salary3d)
        salary22 = np.where((initial_p_aep==6) & (self.treatment == 0) & (self.typeSchool == 0), sum([4*RBMNSecond,ExpTrameS,BRPWithout,AsigSecond]), salary3d)
        
        salary23 = np.where((initial_p_aep==7) & (self.treatment == 0) & (self.typeSchool == 1), sum([4*RBMNElemt,ExpTrameE,BRPWithout,AsigElemt,priorityaep,AcreditaTramoI]), salary3d)
        salary24 = np.where((initial_p_aep==7) & (self.treatment == 0) & (self.typeSchool == 0), sum([4*RBMNSecond,ExpTrameS,BRPWithout,AsigSecond,priorityaep,AcreditaTramoI]), salary3d)
        
        salary25 = np.where((initial_p_aep==8) & (self.treatment == 0) & (self.typeSchool == 1), sum([4*RBMNElemt,ExpTrameE,BRPWithout,AsigElemt,priorityaep,AcreditaTramoII]), salary3d)
        salary26 = np.where((initial_p_aep==8) & (self.treatment == 0) & (self.typeSchool == 0), sum([4*RBMNSecond,ExpTrameS,BRPWithout,AsigSecond,priorityaep,AcreditaTramoII]), salary3d)
        
        salary27 = np.where((initial_p_aep==9) & (self.treatment == 0) & (self.typeSchool == 1), sum([4*RBMNElemt,ExpTrameE,BRPWithout,AsigElemt,priorityaep,AcreditaTramoIII]), salary3d)
        salary28 = np.where((initial_p_aep==9) & (self.treatment == 0) & (self.typeSchool == 0), sum([4*RBMNSecond,ExpTrameS,BRPWithout,AsigSecond,priorityaep,AcreditaTramoIII]), salary3d)
